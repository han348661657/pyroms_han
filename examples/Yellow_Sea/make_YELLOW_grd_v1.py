...
# datadir = 'examples/Yellow_Sea/data/'
datadir = 'data/'
...
lonx=lon; lonx.shape=[-1,1]
latx=lat; latx.shape=[-1,1]
lonlat=np.append(lonx,latx,axis=1)# n*2 np.array
topox=topo;topox.shape=[-1,1] # n*1 np.array
# scipy 中的gridata要比plt中的快n倍
# 插值算法有：{‘linear’, ‘nearest’, ‘cubic’}
h = griddata(lonlat, topox, (hgrd.lon_rho, hgrd.lat_rho),
             method='cubic').squeeze()

# h = griddata((lon.flat, lat.flat), topo.flat,
# hgrd.lon_rho, hgrd.lat_rho)
...
# smooth the raw bathy using the direct iterative method from Martinho and
# Batteen (2006)
rx0_max = 0.35
